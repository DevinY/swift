...
class DCalendar {
    ...
    //用nsDate取下個月的第一天的compent()
    func getNextMonthByNsdate(nd:NSDate)->NSDateComponents{
        var cmc = calendar.components(componentFlags, fromDate: nd)
    ...
    func getPreMonthByNsdate(nd:NSDate)->NSDateComponents{
        var preDate = self.dateBySubtractingDays(1, date: nd)
        var c=calendar.components(componentFlags, fromDate: preDate)
        var firstdayofmonth=formatter.dateFromString("\(c.year)-\(c.month)-01")!
        println("getPreMonthByNsdate:\(c.year)-\(c.month)-01")
        c=calendar.components(componentFlags, fromDate: firstdayofmonth)
        return c
    }
